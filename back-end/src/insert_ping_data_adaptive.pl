...
foreach my $thisfile (@currfiles) {
    ...
    #File name must of format: pings.isi-ant.A.YYYY.MM.DD.hh.mm.ss
       ...
       #split line into variables seperated by a '|'
        ...
        if ( !$result[0] ) {             #not found id in database
           my $record = $gi->record_by_addr($Block);
            ...
            my $start = $OutageStart;
            my $end   = $OutageStart+$Duration;
            ...
            $dbq_PingOutage->execute( $DataSetID_insert,$Block . '/24',$VantageStatus,$Uncertainty,$BlockAggr, $start,
                $end );
